...
public class JCTermMIDP extends MIDlet implements Runnable, CommandListener {
	...
	private void clearKey() {
		try {
			RecordStore.deleteRecordStore(".ssh");
	...
	public void commandAction(Command c, Displayable s) {
		...
		if (c == connectCommand && !isPaused()) {
			...
			if (_host == null || _host.length() == 0 || _user == null || _user.length() == 0) {
				return;
			}
			host = _host;
			user = _user;
	...
	class MyCanvas extends Canvas implements Term {
		...
		public void draw_cursor() {
			/*
			 * cursor_graphics.fillRect(x, y-char_height, char_width, char_height); Graphics
			 * g=getGraphics(); g.setClip(x, y-char_height, char_width, char_height);
