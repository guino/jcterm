...
public class JCTermSWT extends Canvas implements PaintListener, ControlListener, DisposeListener, KeyListener, Term {
	...
	private final Object[] colors = { black, red, green, yellow, blue, magenta, cyan, white };
	...
	private void setSize(Display display, int w, int h) {
		...
		synchronized (this) {
			...
			/*
			 * if(xor_not_supported){ if(graphics.getTextAntialias()!=SWT.OFF)
	...
	private void repaint(final int x, final int y, final int width, final int height) {
		Display display = Display.getDefault();
		if (display == null)
			return;
	...
	public void keyPressed(KeyEvent e) {
		...
		/*
		 * SWT.SHIFT SWT.ALT SWT.ARROW_DOWN SWT.ARROW_LEFT SWT.ARROW_RIGHT SWT.ARROW_UP
		 * SWT.BS SWT.CONTROL / SWT.CTRL SWT.CTRL SWT.DEL SWT.ESC SWT.F1 - SWT.F12
		 * SWT.CR <- RETURN
		 */
