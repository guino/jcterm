...
public class JCTermJ2MECDC extends Panel implements KeyListener, ActionListener, Runnable, Term {
	...
	public void clear_area(int x1, int y1, int x2, int y2) {
		...
		/*
		 * Graphics g=getGraphics(); g.setColor(bground); g.fillRect(x1, y1, x2-x1,
		 * y2-y1); g.setColor(fground);
		 */
	...
	public void setPortForwardingL(int port1, String host, int port2) {
		try {
			session.setPortForwardingL(port1, host, port2);
	...
	public void setPortForwardingR(int port1, String host, int port2) {
		try {
			session.setPortForwardingR(port1, host, port2);
		} catch (JSchException e) {
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 * action.equals("Open EXEC Session... return") ){ try{ port=22; String
		 * _host=textf.getText(); String _user=_host.substring(0, _host.indexOf('@'));
		 * _host=_host.substring(_host.indexOf('@')+1); if(_host==null ||
		 * _host.length()==0){ return; } if(_host.indexOf(':')!=-1){ try{
		 * port=Integer.parseInt(_host.substring(_host.indexOf(':')+1)); }
		 * catch(Exception eee){} _host=_host.substring(0, _host.indexOf(':')); }
		 * user=_user; host=_host;
		 *
	...
	public MenuBar getMenuBar() {
		 ...
		 * mi.setActionCommand("X11 Forwarding..."); m.add(mi);
		 */
		mb.add(m);

		/*
		 * m=new Menu("Etc"); mi=new MenuItem("Compression...");
		 * mi.addActionListener(this); mi.setActionCommand("Compression..."); m.add(mi);
		 * // mi=new MenuItem("Line Space..."); // mi.addActionListener(this); //
		 * mi.setActionCommand("Line Space..."); // m.add(mi); mb.add(m);
		 */
